  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
        {/* Toggle */}
        <div className="md:col-span-4 bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs flex items-center justify-between">
          <span className="text-xs font-bold text-slate-700">Format ng Output:</span>
          <div className="flex bg-slate-100 p-1 rounded-xl">
            <button
              onClick={() => setExportFormat('SQL')}
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                exportFormat === 'SQL' ? 'bg-emerald-700 text-white shadow-xs' : 'text-slate-600'
              }`}
            >
              SQL INSERTs
            </button>
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                exportFormat === 'JSON' ? 'bg-emerald-700 text-white shadow-xs' : 'text-slate-600'
              }`}
            >
              JSON Data
            </button>
